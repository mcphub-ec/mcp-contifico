...
Servidor Model Context Protocol (MCP) para la integración con **el sistema ERP Contífico**.
...
-   Integración completa con el ERP Contífico.
...
-   `listar_personas`: Gestión de clientes (necesario antes de facturar).
...
docker run -d \
  --name mcp-contifico \
  --env-file .env \
  ghcr.io/mcphub-ec/mcp-contifico:latest
```
...
services:
  mcp-contifico:
    image: ghcr.io/mcphub-ec/mcp-contifico:latest
    container_name: mcp-contifico
    env_file:
      - .env
